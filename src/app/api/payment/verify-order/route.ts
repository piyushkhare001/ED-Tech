...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!buyer || !course) {
      return NextResponse.json(
        { success: false, message: "User or Course not found" },
        { status: 404 }
      );
    }
    ...
    const expectedSignature = crypto
      .createHmac("sha256", process.env.RAZORPAY_SECRET_ID || "fallbackSecret")
    ...
    const isAlreadyEnrolled = buyer.courses.some(
    ...
    await mailSender({
      ...
      body: paymentCompletedReceiptTemplate(
        course.title,
        razorpay_payment_id,
