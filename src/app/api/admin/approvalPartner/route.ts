...
export async function POST(req: Request) {
  try {
    ...
    if (
      !email ||
      !adminApproval ||
      !["Approved", "Rejected"].includes(adminApproval)
    ) {
      return NextResponse.json(
    ...
    await mailSender({
      ...
      body: studentPartnerStatusTemplate(
        adminApproval == "Approved" ? "approved" : "denied"
      ),
    });
    ...
    return NextResponse.json(
      {
        success: true,
        message: `Student partner application ${adminApproval}`,
      },
      { status: 200 }
    );
  } catch (error: any) {
    console.error(
      "Error approving or rejecting student partner application:",
      error
    );
    return NextResponse.json(
